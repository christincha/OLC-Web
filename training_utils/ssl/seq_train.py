...
from training_utils.ssl.utilities import save_checkpoint
from training_utils.ssl.data_loader import *
from training_utils.ssl.extract_hidden import extract_hidden_ordered
...
def clustering_knn_acc(model, train_loader, hidden_size,
                       alpha, epoch,
                       device, reducer_name, dimension, return_full=False
                       ):
    """Computes hidden states of keypoint sequences in the data_loader, performs dimension reduction, and plots results
        Inputs:
            ...
            return_full: return the features before performing dimension reduction
    """
    if alpha == 0:
        hi_train, label_train,  train_semi = extract_hidden_ordered(model, train_loader, hidden_size, alpha, device)
    else:
        hi_train, label_train, train_semi, pre_label_list, mi = extract_hidden_ordered(model, train_loader, hidden_size, alpha, device)
    ...
    if return_full:
        if alpha == 0:
            return hi_train, train_semi
        else:
            return hi_train, train_semi, mi
    if alpha == 0:
        print('return alpha 0')
        return transformed, train_semi
    else:
        print('return alpha 0.5')
        return [transformed, train_semi, mi]
